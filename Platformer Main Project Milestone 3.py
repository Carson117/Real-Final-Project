...
class Game:
    ...
    def run(self):
        ...
        while self.playing:   # This while statement checks events, update, and draw each time the game is running
            self.clock.tick(FPS)
    ...
    def update(self):
        ...
        if self.player.rect.top <= HEIGHT / 4:
            ...
            for plat in self.platforms: # taking the platforms and moving them down
                plat.rect.y += abs(int(self.player.vel.y)) # moving platform down at whatever speed the player is going
        ...
        while len(self.platforms) < 6: # 5 starting, 1 above
            width = random.randrange(50, 150) # reandomize the width
            p = Platform(random.randrange(0, WIDTH-width),
                        random.randrange(-75, -30),
                        width, 20) # spawn new platform
    ...
    def draw_text(self, text, size, color, x, y):
        ...
        text_rect.midtop = (int(x),int(y)) # placement of text
...
while g.running:  #loop for running the game
    g.new() # class new to set up a new game
